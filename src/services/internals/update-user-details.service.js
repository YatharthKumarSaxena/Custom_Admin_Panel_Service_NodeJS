 ...
 * @param {string} firstName - The new firstName value
...
const updateUserDetailsService = async (userId, type, firstName, executedBy, requestId) => {
  try {
    ...
    logWithTime(`🔄 Updating first name for ${type}: ${userId} to ${firstName}`);
    ...
    const action = "UPDATE_USER_DETAILS";
    logWithTime(`✅ Successfully updated user details for ${type}: ${userId}`);
    ...
    logServiceTrackerEvent({
      ...
      description: `${type} details updated with ${idField} ${userId}`,
      targetId: userId,
      executedBy: executedBy,
      metadata: { 
        type,
    ...
    logServiceTrackerEvent({
      ...
      description: `Unexpected error during update user details: ${err.message}`,
